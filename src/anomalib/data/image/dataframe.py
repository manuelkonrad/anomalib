...
def make_dataframe_dataset(
    ...
    Args:
        samples (pd.DataFrame): DataFrame containing the dataset information.
    ...
    Examples:
        Assume that we would like to use this ``make_dataframe_dataset`` to create a dataset from a DataFrame.
...
class DataframeDataset(AnomalibDataset):
    ...
    This class is used to create a dataset from a DataFrame. The class utilizes the Torch Dataset class.
...
class Dataframe(AnomalibDataModule):
    ...
    Examples:
        The following code demonstrates how to use the ``Dataframe`` datamodule. Assume that the DataFrame is structured
        as follows:
